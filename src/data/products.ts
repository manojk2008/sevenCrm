...
const units = ['Per License', 'Per User', 'Per Month', 'Per Year', 'Per Project', 'Per Hour'];
...
export const products: Product[] = productNames.map((name, i) => ({
  ...
  hsnCode: `9983${10 + i}`,
  gstRate: 18,
  sku: `SKU-2024-${String(i + 1).padStart(3, '0')}`,
  unit: units[i % units.length],
  stock: i % 3 === 1 ? 0 : 10 + i, // Services shown with 0 stock rather than null — see note below
  status: 'active',
  images: [],
  specifications: [
    { key: 'License Type', value: i % 2 === 0 ? 'Annual' : 'Perpetual' },
    { key: 'Support Included', value: '24x7 Email & Call' },
  ],
